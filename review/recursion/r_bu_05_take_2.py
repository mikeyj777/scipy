...
def betting_until_payday(bankroll=bankroll, target=target, p=p, win = True):
  ...
  if True in bankroll_prob_dict[bankroll]:
    win_ans = bankroll_prob_dict[bankroll][True]
  else:
    win_ans = p * betting_until_payday(bankroll=bankroll + 1, win=True)
    bankroll_prob_dict[bankroll][True] = win_ans
  
  lose_ans = 0
  if False in bankroll_prob_dict[bankroll]:
    lose_ans = bankroll_prob_dict[bankroll][False]
  else:
    lose_ans = (1-p) * betting_until_payday(bankroll=bankroll - 1, win=False)
    bankroll_prob_dict[bankroll][False] = lose_ans
  if True in bankroll_prob_dict[bankroll] and False in bankroll_prob_dict[bankroll]:
    bankroll_prob_dict[bankroll]['ans'] = bankroll_prob_dict[bankroll][True] + bankroll_prob_dict[bankroll][False]
  return bankroll_prob_dict[bankroll][win]
